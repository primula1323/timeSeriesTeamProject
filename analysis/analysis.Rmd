...
```{r}
ts_data <- to_ts('adj_student')
real_data <- to_ts_test('adj_student', df_test)
plot(ts_data)
```
...
```{r}
ts_data <- to_ts('adj_adult')
real_data <- to_ts_test('adj_adult', df_test)
plot(ts_data)
...
```{r}
analyze_stationarity <- function(ts_data, column_name) {
  print(paste("--- Stationarity Analysis for:", column_name, "---"))
  plot(ts_data, main = paste("Time Series Plot for", column_name))
  plot(decompose(ts_data, type="additive"), main = paste("Decomposition for", column_name)) # 또는 multiplicative
  
  print("ADF Test Results:")
  print(adf.test(ts_data))
  
  # ACF/PACF plots
  acf(ts_data, main = paste("ACF for", column_name))
  pacf(ts_data, main = paste("PACF for", column_name))
  
  # 필요시 차분 및 재검토 로직 추가
  # ...
  # return(차분된_시계열_또는_원본_시계열)
}

analyze_stationarity(ts_data, 'adj_adult')
```
